...
Venda.Attributes.generateSwatch = function(attributeNumber, uID){
	for(var i = 0; i < Venda.Attributes.productArr.length; i++) {
		...
		for (var t = 0; t < Venda.Attributes.productArr[i].attSet[attributeNumber].optionValues.length; t++) {
			swatchList += '<li class="'+ (Venda.Attributes.productArr[i].attSet[attributeNumber].optionValues[t]).replace(/[' ]/g,"_") +' attributeSwatch" id="attributeSwatch_' + uID + '" data-attName="'+ attributeNumber +'" data-attValue="'+ Venda.Attributes.productArr[i].attSet[attributeNumber].optionValues[t] +'"><span class="swatchText">' + Venda.Attributes.productArr[i].attSet[attributeNumber].options[t] + '</span></li>';
		}
...
Venda.Attributes.initViewLargeImagePopup = function() {
	...
	/* This has been removed until we have a better solution for touch detection
