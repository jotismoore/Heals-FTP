...
Venda.Validate.errorPlacement = function(error, element) {
    if (jQuery(element).is("select")|| jQuery(element).attr('id') === 'q') {
...
Venda.Validate.highlight = function(element, errorClass, validClass) {
    var $elem = jQuery(element);
    var $eleType = Venda.Validate.getFieldType(element);

    if ($eleType == "checkbox" || $eleType == "radio") {
        this.findByName($elem.attr('name')).addClass(errorClass).removeClass(validClass);
    } else if ($elem.is("select") || $elem.attr('id') === 'q') {
        $elem.closest("div").addClass(errorClass).removeClass(validClass);
    } else {
        $elem.addClass(errorClass).removeClass(validClass);
    }
};

Venda.Validate.unhighlight = function(element, errorClass, validClass) {
    var $elem = jQuery(element);
    var $eleType = Venda.Validate.getFieldType(element);

    if ($eleType == "checkbox" || $eleType == "radio") {
        this.findByName($elem.attr('name')).removeClass(errorClass).addClass(validClass);
    } else if ($elem.is("select") || $elem.attr('id') === 'q') {
        $elem.closest("div").removeClass(errorClass).addClass(validClass);
    } else {
        $elem.removeClass(errorClass).addClass(validClass);
    }
};

Venda.Validate.getFieldType = function(element){
    return jQuery(element).attr("type");
};
